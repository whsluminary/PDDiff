...
class DDIM(nn.Module):
    # classic DDIM with Gaussian diffusion, in image space
    def __init__(self,
                 denoise,
                 condition,
                 timesteps=100,
    ...
    def register_schedule(self, given_betas=None, beta_schedule="linear", timesteps=1000,
        ...
        posterior_variance = self.eta * self.eta * (1. - alphas_cumprod_prev) * (1 - alphas) / (1 - alphas_cumprod)
        ...
        self.register_buffer('predicted_x_start', to_torch(
            np.sqrt(alphas_cumprod_prev) -
            np.sqrt((1 - alphas_cumprod_prev - posterior_variance) * alphas_cumprod / (1 - alphas_cumprod))))
        self.register_buffer('direction', to_torch(
            np.sqrt((1 - alphas_cumprod_prev - posterior_variance) / (1 - alphas_cumprod))))
    ...
    def q_posterior(self, x_start, x_t, t):
        ...
        posterior_mean = (
                extract_into_tensor(self.predicted_x_start, t, x_t.shape) * x_start +
                extract_into_tensor(self.direction, t, x_t.shape) * x_t
        )
